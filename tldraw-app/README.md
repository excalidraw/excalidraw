...
1. Reuses the existing backend at `VITE_TERRAFORM_BACKEND_URL` (defaults to
   `http://localhost:3000`).
2. Calls `GET /terraform/upload/:id/render/tldraw` to get a backend-generated
   tldraw document.
3. Loads returned `shapes` into `<Tldraw />` via `editor.createShapes(...)`.

The backend connector currently emits shape partials from `DiagramIR` using a
deterministic grid layout and relationship arrows. This keeps the format stable
for clients while the connector evolves toward richer tldraw-native documents.
