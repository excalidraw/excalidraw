...
def main() -> None:
    ...
    # Must run from the graph-layout-rag tool root
    manifest_path = Path("data/manifest.json")
    if not manifest_path.exists():
        sys.exit("Run from tools/graph-layout-rag/ — data/manifest.json not found")

    # Inline import after path check so errors are clear
    from graph_layout_rag.harvest.bibliography import _doi_relevance_decision
    from graph_layout_rag.harvest.parallel import parallel_map
    from graph_layout_rag.harvest.relevance import is_known_offtopic_doi
