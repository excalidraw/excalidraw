...
from graph_layout_rag.doc_vectors import embedding_scores, has_doc_vectors
from graph_layout_rag.eval.metrics import aggregate_metrics, case_metrics
from graph_layout_rag.query.citation_rank import CitationGraph, load_graph, rank_related
...
def run_related_eval(
    *, variants: list[str] | None = None, max_cases: int = 150, min_refs: int = 1,
    ...
    try:
        ...
        db.close()
    if not g.doc_to_oa:
        raise RuntimeError("Citation graph is empty. Run: graph-layout-rag cite enrich")
