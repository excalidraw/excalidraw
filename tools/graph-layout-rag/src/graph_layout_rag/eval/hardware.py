...
def assert_memory_start_safe(min_available_gb: float) -> MemorySnapshot:
    ...
    if os.getenv("GRAPH_RAG_ALLOW_INGEST_DURING_EVAL", "").lower() not in ("1", "true", "yes"):
        ingest = _run_text("pgrep", "-f", "graph-layout-rag ingest")
        if ingest:
            raise RuntimeError("graph-layout-rag ingest is running; stop it before benchmarking")
    return snapshot
