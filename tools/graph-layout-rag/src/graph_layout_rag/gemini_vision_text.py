...
full-corpus ingest. Bound it with ``GRAPH_RAG_GEMINI_VISION_MAX_PAGES``.
...
log = logging.getLogger("graph_layout_rag.gemini_vision")
...
def _vision_model() -> str:
    return os.getenv("GRAPH_RAG_GEMINI_VISION_MODEL", DEFAULT_MODEL).strip() or DEFAULT_MODEL


def _vision_location() -> str:
    return (
        os.getenv("GRAPH_RAG_GEMINI_VISION_LOCATION", DEFAULT_LOCATION).strip()
        or DEFAULT_LOCATION
    )
...
def _vision_dpi() -> int:
    try:
        return int(os.getenv("GRAPH_RAG_GEMINI_VISION_DPI", str(DEFAULT_DPI)))
    except ValueError:
        return DEFAULT_DPI
...
def _max_pages() -> int:
    """0 = all pages."""
    try:
        return max(0, int(os.getenv("GRAPH_RAG_GEMINI_VISION_MAX_PAGES", "0")))
    except ValueError:
        return 0
...
def _min_interval_s() -> float:
    try:
        return max(0.0, int(os.getenv("GRAPH_RAG_GEMINI_VISION_MIN_INTERVAL_MS", "0")) / 1000.0)
    except ValueError:
        return 0.0
...
def _concurrency() -> int:
    ...
    """
    try:
        requested = int(os.getenv("GRAPH_RAG_GEMINI_VISION_CONCURRENCY", str(DEFAULT_CONCURRENCY)))
    except ValueError:
        return DEFAULT_CONCURRENCY
    clamped = max(1, min(requested, MAX_CONCURRENCY))
    if requested > MAX_CONCURRENCY:
        log.warning(
            "GRAPH_RAG_GEMINI_VISION_CONCURRENCY=%d exceeds the hard cap %d; clamping",
...
def _timeout_ms() -> int:
    ...
    try:
        return max(0, int(os.getenv("GRAPH_RAG_GEMINI_VISION_TIMEOUT_MS", str(DEFAULT_TIMEOUT_MS))))
    except ValueError:
