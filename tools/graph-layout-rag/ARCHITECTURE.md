...
Agent-oriented reference for the graph-drawing literature RAG tool at `tools/graph-layout-rag/`. Covers harvest, PDF extraction, ingestion, hybrid query, retrieval evaluation, catalog classification, testing, and operational constraints.

**Related docs:**

- User-facing setup: [README.md](./README.md)
- Agent query workflow: [.agents/skills/graph-layout-rag/SKILL.md](../../.agents/skills/graph-layout-rag/SKILL.md)
- Pipeline layout query pack: [docs/pipeline-rag-queries.md](../../docs/pipeline-rag-queries.md)
- Retrieval evaluation report (when generated): [docs/graph-layout-rag-retrieval-evaluation.md](../../docs/graph-layout-rag-retrieval-evaluation.md)
...
- Uses env prefix `GRAPH_RAG_` for tool-specific overrides
...
Package root: `tools/graph-layout-rag/` (`PKG_ROOT` in `paths.py`).

```
tools/graph-layout-rag/
├── src/graph_layout_rag/     # Python package
...
**Override:** set `GRAPH_RAG_INDEXES_DIR` to relocate all profile indexes.

### Env file load order (`env.py`)

1. `tools/graph-layout-rag/.env`
...
| `graph-layout-rag harvest status` | Status dump | Manifest + checkpoint + ledger summary |
| `graph-layout-rag harvest report` | Ledger query | Per-URL attempt log from SQLite |
| `graph-layout-rag harvest prune` | `plan_prune` / `apply_prune` | Precision-first corpus cleanup (destructive with `--apply`) |
| `graph-layout-rag harvest enrich` | `enrich_manifest` | Backfill missing abstracts via OpenAlex (no downloads) |
...
```
                    ┌─────────────────────────────────────┐
                    │  graph-layout-rag harvest [--flags] │
                    └─────────────────┬───────────────────┘
                                      │
                    ┌─────────────────▼───────────────────┐
...
CLI: `graph-layout-rag harvest prune` (dry plan) or `harvest prune --apply --yes` (destructive). Backs up manifest before apply.
...
Used by DOI resolver and archive fallbacks (not PDF downloads). 5 attempts; User-Agent includes `mailto:graph-layout-rag@excalidraw-tf.local`.
...
producer thread consumes completed PDF futures out of order:
  put ExtractionOutcome into bounded GRAPH_RAG_EXTRACT_QUEUE_DOCS queue

main ingest thread consumes queued outcomes:
  accumulate in batch (GRAPH_RAG_INGEST_DOC_BATCH docs)
...
Set via `GRAPH_RAG_PDF_BACKEND`. Changing backend invalidates both dense and BM25 indexes (different extracted text) → requires `ingest --force --rebuild`.
...
- `GRAPH_RAG_EXTRACT_WORKERS=4` by default; `0` or `1` uses serial extraction
...
- Render pages to PNG via PyMuPDF (`GRAPH_RAG_GEMINI_VISION_DPI`, default 200)
- Transcribe to GFM Markdown via Gemini (default `gemini-3.1-pro-preview`)
- Async concurrent transcription (`GRAPH_RAG_GEMINI_VISION_CONCURRENCY`, default 8; hard cap 200)
- `GRAPH_RAG_GEMINI_VISION_MAX_PAGES=0` → all pages
...
**Batch size:** code default **25** (`GRAPH_RAG_INGEST_DOC_BATCH`).
...
**Progress logging:** `data/ingest.log`; stderr shows `embed progress: N/M texts (+Xs, total Ys)`. Scan progress every `GRAPH_RAG_INGEST_SCAN_LOG_EVERY` items (default 100).
...
| `GRAPH_RAG_DOCLING_TABLES` | `1` | Enable Docling table structure extraction |
| `GRAPH_RAG_DOCLING_TIMEOUT_S` | `600` | Per-document Docling timeout |
| `GRAPH_RAG_DOCLING_DEVICE` | `auto` | `auto`, `cpu`, `cuda`, `mps`, or `xpu` |
| `GRAPH_RAG_DOCLING_THREADS` | `2` | Accelerator threads in each Docling worker |
...
- Model: `GRAPH_RAG_EVAL_LLM_MODEL` (default `gemini-2.0-flash`)
...
```bash
uv run graph-layout-rag eval retrieval \
  --embed-profile gemini-2-structure-v1 \
...
**Entry point:** `graph-layout-rag` → `cli.py:main`
...
| `GRAPH_RAG_GEMINI_VISION_MAX_PAGES` | 0 | Page cap per doc (0 = all) |
| `GRAPH_RAG_GEMINI_VISION_CONCURRENCY` | 8 | Async page pool (hard cap 200) |
| `GRAPH_RAG_GEMINI_VISION_TIMEOUT_MS` | 120000 | Per-request timeout |
| `GRAPH_RAG_GEMINI_VISION_COST_PER_MTOK` | 2.0 | Cost estimate for compare-extract |
...
| `GRAPH_RAG_INGEST_PROGRESS_LOG_INTERVAL_S` | 30 | Emit progress/ETA INFO log after this many seconds |
| `GRAPH_RAG_WORKERS` | 2 (OpenAI) / 4 (local) | Parallel embed workers |
| `GRAPH_RAG_INDEXES_DIR` | — | Override indexes root path |
...
```bash
cd tools/graph-layout-rag
uv run graph-layout-rag harvest --resume -v 2>&1 | tee -a data/harvest.log
```
...
```bash
caffeinate -dims uv run graph-layout-rag ingest -v
```

### 11.5 A/B embed profiles

```bash
RAG_EMBED_PROFILE=gemini-2-structure-v1 uv run graph-layout-rag ingest --force --rebuild -v
RAG_EMBED_PROFILE=mlx-qwen4b uv run graph-layout-rag ingest --force --rebuild -v

uv run graph-layout-rag query "Sugiyama layering" --embed-profile gemini-2-structure-v1 --json
uv run graph-layout-rag query "Sugiyama layering" --embed-profile mlx-qwen4b --json
uv run graph-layout-rag embed indexes
uv run graph-layout-rag eval benchmark --embed-profile gemini-2-structure-v1 --no-llm-transforms -v
```
...
Agents with filesystem access may also read the PDF directly at `tools/graph-layout-rag/{localPath}`.
...
- Lower `GRAPH_RAG_INGEST_DOC_BATCH` below 25 on memory-constrained Macs
...
- Changing `GRAPH_RAG_PDF_BACKEND` invalidates both dense and BM25 text
