...
TRUSTED_VENUE_CHECKPOINT_PATH = DATA_DIR / "trusted_venue_checkpoint.json"
...
def indexes_root() -> Path:
    raw = os.getenv("RAG_LIT_INDEXES_DIR", "").strip()
    if raw:
        path = Path(raw)
...
def resolve_profile_name(explicit: str | None = None) -> str:
    ...
    name = _resolve_profile_name_env(prefix="RAG_LIT_", profile=explicit)
