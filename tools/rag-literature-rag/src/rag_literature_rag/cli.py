...
from rag_literature_rag.catalog.classify import build_catalog, summarize_catalog
from rag_literature_rag.catalog.taxonomy import PIPELINE_CATEGORIES, UNCATEGORIZED
from rag_literature_rag.harvest.run import harvest_group
from rag_literature_rag.ingest.run import ingest_cmd
from rag_literature_rag.query.search import search
from rag_literature_rag.query.retrieve import DEFAULT_HYBRID

@click.group()
def main() -> None:
    """RAG methodology literature RAG — harvest, ingest, query."""
...
from rag_literature_rag.eval.retrieval import retrieval_eval_cmd  # noqa: E402
...
from rag_literature_rag.eval.pool_commands import diagnostics_cmd, judge_cmd, pool_cmd  # noqa: E402
...
def eval_related_cmd(
    ...
    from rag_literature_rag.eval.related_eval import run_related_eval, write_related_report
...
def cite_enrich_cmd(force: bool, workers: int, no_s2: bool, incoming: bool, incoming_cap: int, incoming_workers: int) -> None:
    ...
    """
    from rag_literature_rag.harvest.cite_enrich import enrich_citations
...
def cite_stats_cmd() -> None:
    ...
    if not cs.CITATIONS_DB_PATH.exists():
        click.echo("No citation store yet. Run: rag-literature-rag cite enrich")
        sys.exit(1)
    db = cs.connect()
...
def cite_embed_related_cmd(model: str, rebuild: bool, batch_size: int) -> None:
    ...
    from rag_literature_rag.doc_vectors import build_doc_vectors
...
def cite_related_cmd(doc_id: str, top: int, signal: str, model: str, as_json: bool) -> None:
    ...
    from rag_literature_rag import citation_store as cs
    from rag_literature_rag.doc_vectors import (
        embedding_scores,
        has_doc_vectors,
        load_all_vectors,
        related_by_embedding,
    )
    from rag_literature_rag.query.citation_rank import load_graph, related_to_docs
    ...
    if signal == "embedding":
        if not has_doc_vectors(model):
            click.echo(f"No {model} doc vectors. Run: rag-literature-rag cite embed-related --model {model}", err=True)
            sys.exit(1)
    ...
    if not cs.CITATIONS_DB_PATH.exists():
        click.echo("No citation store yet. Run: rag-literature-rag cite enrich", err=True)
        sys.exit(1)
    db = cs.connect()
    graph = load_graph(db)
...
def embed_profiles_cmd(as_json: bool) -> None:
    ...
    from rag_literature_rag.ingest.embed import list_embed_profiles
...
def embed_indexes_cmd(as_json: bool) -> None:
    ...
    from rag_literature_rag.ingest.index import describe_profile_index
    from rag_literature_rag.paths import list_profile_indexes
...
def query_cmd(
    ...
    if not results:
        click.echo("No results. Run: rag-literature-rag harvest && rag-literature-rag ingest")
        sys.exit(1)
