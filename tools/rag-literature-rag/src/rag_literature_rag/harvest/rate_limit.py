...
def wait_for_domain(domain: str) -> None:
    ...
    with lock:
        ...
        # Reserve a slot for this caller; fire_at is when this thread may proceed.
        # Sleeping happens OUTSIDE the lock so parallel workers sleep concurrently
        # instead of serializing through here for the full sleep duration.
        fire_at = max(now, last + gap)
        _domain_last[domain] = fire_at
