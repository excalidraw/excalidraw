...
from rag_literature_rag.harvest.rate_limit import parse_retry_after
...
class OpenAlexPolicy(ProviderPolicy):
    ...
    def __init__(self, **kwargs: Any) -> None:
        ...
        self._daily_budget = _env_float("RAG_LIT_OPENALEX_FREE_BUDGET_USD", 1.0)
...
OPENALEX = OpenAlexPolicy(
    concurrency=_env_int("RAG_LIT_OPENALEX_CONCURRENCY", 32),
    rps=_env_float("RAG_LIT_OPENALEX_RPS", 100.0),
)
SEMANTIC_SCHOLAR = ProviderPolicy(
    "semantic-scholar",
    concurrency=_env_int("RAG_LIT_S2_CONCURRENCY", 32),
    rps=_env_float("RAG_LIT_S2_RPS", 1000.0),
    cooldown_seconds=_env_float("RAG_LIT_S2_COOLDOWN_SECONDS", 300.0),
)
CORE = ProviderPolicy(
    "core",
    concurrency=_env_int("RAG_LIT_CORE_CONCURRENCY", 2),
    rps=_env_float(
        "RAG_LIT_CORE_RPS",
