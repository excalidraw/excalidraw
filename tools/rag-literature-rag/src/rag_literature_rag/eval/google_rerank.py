...
def _cache_path() -> Path:
    raw = os.getenv("RAG_LIT_GOOGLE_RANKING_CACHE", "").strip()
...
def _consume_unit() -> None:
    global _units_used
    limit = int(
        os.getenv("RAG_LIT_MAX_CLOUD_RANKING_UNITS", str(DEFAULT_MAX_RANKING_UNITS))
    )
    with _lock:
...
def rerank_google(
    ...
    *,
    ...
    if os.getenv("RAG_LIT_ALLOW_CLOUD_COST", "").lower() not in ("1", "true", "yes"):
