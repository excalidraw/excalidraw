...
from rag_literature_rag import gemini_vision_text
...
def test_transcribes_with_fake_client(tmp_path: Path, monkeypatch):
    pdf = _one_page_pdf(tmp_path)
    monkeypatch.setenv("RAG_LIT_GEMINI_VISION_CONCURRENCY", "1")  # exercise the sync path
    gemini_vision_text._client.cache_clear()
...
def test_clean_false_skips_normalization(tmp_path: Path, monkeypatch):
    pdf = _one_page_pdf(tmp_path)
    monkeypatch.setenv("RAG_LIT_GEMINI_VISION_CONCURRENCY", "1")
    gemini_vision_text._client.cache_clear()
    monkeypatch.setattr(
        gemini_vision_text,
        "_transcribe_page",
...
def test_parallel_path_transcribes_all_pages_in_order(tmp_path: Path, monkeypatch):
    ...
    monkeypatch.setenv("RAG_LIT_GEMINI_VISION_CONCURRENCY", "4")  # async pool path
    gemini_vision_text._client.cache_clear()
    counter: dict = {}
    monkeypatch.setattr(
...
def test_concurrency_clamped_to_hard_cap(monkeypatch):
    monkeypatch.setenv("RAG_LIT_GEMINI_VISION_CONCURRENCY", "9999")
    assert gemini_vision_text._concurrency() == gemini_vision_text.MAX_CONCURRENCY
    monkeypatch.setenv("RAG_LIT_GEMINI_VISION_CONCURRENCY", "0")
    assert gemini_vision_text._concurrency() == 1  # floor
    monkeypatch.setenv("RAG_LIT_GEMINI_VISION_CONCURRENCY", "16")
...
def test_backend_routes_to_gemini(tmp_path: Path, monkeypatch):
    ...
    monkeypatch.setattr(
        "rag_literature_rag.gemini_vision_text.extract_pages_gemini",
...
def test_default_pdf_backend_accepts_gemini(monkeypatch):
    monkeypatch.setenv("RAG_LIT_PDF_BACKEND", "gemini")
