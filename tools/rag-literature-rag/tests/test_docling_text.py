...
from rag_literature_rag import docling_text
from rag_literature_rag.ingest import extract as extract_mod
...
def test_backend_routes_to_docling(tmp_path: Path, monkeypatch):
    ...
    monkeypatch.setattr(
        "rag_literature_rag.docling_text.extract_pages_docling", stub
    )
...
def test_backend_routes_to_pymupdf(tmp_path: Path, monkeypatch):
    ...
    monkeypatch.setattr(
        "rag_literature_rag.ingest.extract.extract_pages_from_path",
...
def test_default_pdf_backend_env(monkeypatch):
    monkeypatch.delenv("RAG_LIT_PDF_BACKEND", raising=False)
    assert extract_mod.default_pdf_backend() == "pymupdf"
    monkeypatch.setenv("RAG_LIT_PDF_BACKEND", "docling")
    assert extract_mod.default_pdf_backend() == "docling"
    monkeypatch.setenv("RAG_LIT_PDF_BACKEND", "bogus")
    assert extract_mod.default_pdf_backend() == "pymupdf"
...
def test_pipeline_options_defaults(monkeypatch):
    for name in (
        "RAG_LIT_DOCLING_OCR",
        "RAG_LIT_DOCLING_TABLES",
        "RAG_LIT_DOCLING_TIMEOUT_S",
        "RAG_LIT_DOCLING_DEVICE",
        "RAG_LIT_DOCLING_THREADS",
    ):
...
def test_pipeline_options_valid_overrides(monkeypatch):
    monkeypatch.setenv("RAG_LIT_DOCLING_OCR", "1")
    monkeypatch.setenv("RAG_LIT_DOCLING_TABLES", "0")
    monkeypatch.setenv("RAG_LIT_DOCLING_TIMEOUT_S", "42")
    monkeypatch.setenv("RAG_LIT_DOCLING_DEVICE", "cpu")
    monkeypatch.setenv("RAG_LIT_DOCLING_THREADS", "7")
...
def test_pipeline_options_invalid_values_fall_back(monkeypatch, caplog):
    monkeypatch.setenv("RAG_LIT_DOCLING_OCR", "sometimes")
    monkeypatch.setenv("RAG_LIT_DOCLING_TABLES", "maybe")
    monkeypatch.setenv("RAG_LIT_DOCLING_TIMEOUT_S", "0")
    monkeypatch.setenv("RAG_LIT_DOCLING_DEVICE", "tpu")
    monkeypatch.setenv("RAG_LIT_DOCLING_THREADS", "many")
    ...
    assert options.accelerator_options.device == "auto"
    assert options.accelerator_options.num_threads == 2
    assert caplog.text.count("invalid RAG_LIT_DOCLING_") == 5
