...
from rag_literature_rag.eval.strategies import CitationGraphStrategy, strategy_registry
...
def test_degrades_to_hybrid_without_store(monkeypatch):
    ...
    monkeypatch.setattr(
        "rag_literature_rag.query.retrieve.retrieve_candidates",
        lambda *a, **k: [dict(r) for r in sentinel],
    )
    monkeypatch.setattr(
        "rag_literature_rag.query.citation_rank.load_graph_cached", lambda: None
    )
