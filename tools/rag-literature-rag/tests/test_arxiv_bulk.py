...
from rag_literature_rag.harvest.arxiv_bulk import _parse_entries, harvest_arxiv_category
...
def test_harvest_category_strict_filters_and_stops_on_empty():
    page0 = _feed(
        _entry("2105.1", "Self-RAG retrieval augmented generation reflection tokens", "RAG"),
        _entry("2105.2", "Smoothed analysis of the simplex method for LP", "optimization"),
    )
    empty = _feed()
    with patch("rag_literature_rag.harvest.arxiv_bulk.httpx.Client") as cc, \
         patch("rag_literature_rag.harvest.arxiv_bulk.time.sleep"):
        client = MagicMock()
    ...
    assert any("Self-RAG" in t for t in titles)
    assert not any("simplex method" in t for t in titles)
