...
from rag_literature_rag.harvest.doi_resolver import (
    _arxiv_pdf_url,
...
def test_pick_pdf_urls_merges_sources():
    ...
    with patch("rag_literature_rag.harvest.doi_resolver._unpaywall_pdf") as unpaywall:
        with patch("rag_literature_rag.harvest.doi_resolver._semantic_scholar_pdf") as s2:
            urls = pick_pdf_urls(
...
def test_pick_pdf_urls_no_hal_or_paywall_by_default():
    with patch("rag_literature_rag.harvest.doi_resolver._unpaywall_pdf", return_value=[]):
        with patch("rag_literature_rag.harvest.doi_resolver._semantic_scholar_pdf", return_value=None):
            urls = pick_pdf_urls(
...
def test_pick_pdf_urls_archive_only_when_requested():
    with patch(
        "rag_literature_rag.harvest.doi_resolver.archive_pdf_urls",
...
def test_resolve_skips_malformed_doi_without_openalex():
    with patch("rag_literature_rag.harvest.doi_resolver._openalex_by_doi") as openalex:
