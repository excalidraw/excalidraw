...
from rag_literature_rag.harvest.checkpoint import discovery_complete
...
def test_relevance_resumes_skips_openalex_for_known_decisions():
    ...
    with patch("rag_literature_rag.harvest.bibliography._openalex_by_doi") as openalex:
        with patch("rag_literature_rag.harvest.bibliography.parallel_map") as pmap:
...
def test_positive_env_int(monkeypatch):
    monkeypatch.setenv("RAG_LIT_TEST_INT", "32")
    assert _positive_env_int("RAG_LIT_TEST_INT", 8) == 32
    monkeypatch.setenv("RAG_LIT_TEST_INT", "invalid")
    assert _positive_env_int("RAG_LIT_TEST_INT", 8) == 8
