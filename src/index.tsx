...
import { capitalizeString, getDateTime, isInputLike } from "./utils";

import { EditableText } from "./components/EditableText";
import { ColorPicker } from "./components/ColorPicker";
import { ButtonSelect } from "./components/ButtonSelect";

import { SHAPES, shapesShortcutKeys, findElementByKey } from "./shapes";
...
class App extends React.Component<{}, AppState> {
  ...
  private onKeyDown = (event: KeyboardEvent) => {
    ...
    if (event.key === KEYS.ESCAPE) {
      ...
      const currentEntry = sceneHistory.generateCurrentEntry(elements);
      if (event.shiftKey) {
        // Redo action
        const entryToRestore = sceneHistory.redoStack.pop();
        if (entryToRestore !== undefined) {
          sceneHistory.restoreEntry(elements, entryToRestore);
          sceneHistory.stateHistory.push(currentEntry);
        }
      } else {
        // undo action
        let lastEntry = sceneHistory.stateHistory.pop();
        // If nothing was changed since last, take the previous one
        if (currentEntry === lastEntry) {
          lastEntry = sceneHistory.stateHistory.pop();
        }
        if (lastEntry !== undefined) {
          sceneHistory.restoreEntry(elements, lastEntry);
          sceneHistory.redoStack.push(currentEntry);
        }
      }
      this.forceUpdate();
      event.preventDefault();
  ...
  componentDidUpdate() {
    ...
    if (sceneHistory.isRecording()) {
      sceneHistory.pushEntry(sceneHistory.generateCurrentEntry(elements));
      sceneHistory.redoStack.splice(0, sceneHistory.redoStack.length);
    }
    sceneHistory.continueRecording();
  }
}
