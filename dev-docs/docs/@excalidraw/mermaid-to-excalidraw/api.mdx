...
This API recieves the mermaid syntax as the input, parses the mermaid syntax and resolves to Excalidraw Skeleton.
You will need to call `convertToExcalidraw` API to convert the element skeleton to excalidraw elements.
...
Since the elements returned from the parser are not fully Excalidraw Elements hence you will need to the call the API `convertToExcalidrawElements` before rendering it to Excalidraw.

This limitation is due to the[@excalidraw/excalidraw](/docs/@excalidraw/excalidraw/installation) being a **UMD** build so we cannot export just the util yet, hence the host will need to keep using this function unless we support **ESM** build.

## Supported Chart Types

Currently only [flowcharts](https://mermaid.js.org/syntax/flowchart.html) are supported. The other chart types will be rendered as an image in Excalidraw.
...
Currently only [flowcharts](https://mermaid.js.org/syntax/flowchart.html) are supported. All other chart types will be rendered as an image in Excalidraw.
