...
For this create a file named `{{diagramType}}.ts` in [`src/converter/types`](https://github.com/excalidraw/mermaid-to-excalidraw/tree/master/src/converter/types) and write a function `parseMermaid{{diagramType}}Diagram` similar to how we [`parseMermaidFlowChartDiagram`](https://github.com/excalidraw/mermaid-to-excalidraw/blob/master/src/converter/types/flowchart.ts#L438) for parsing flowchart diagram.
