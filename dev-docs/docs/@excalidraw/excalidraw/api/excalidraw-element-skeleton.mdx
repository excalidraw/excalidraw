...
For this purpose we introduced a format [`ExcalidrawElementSkeleton`](https://github.com/excalidraw/excalidraw/blob/master/src/data/transform.ts#L133). This format is nothing but a simplified version of [`ExcalidrawElement`](https://github.com/excalidraw/excalidraw/blob/master/src/element/types.ts#L134) with minimum possible attributes so users can create the individual elements programmatically, bind arrows and create text containers easily by supplying the minimum required attributes and rest all is taken care by Excalidraw Library.
...
To create these shapes you need to pass its `type` and `x` and `y` coordinates for position. Rest all attributes are _optional_.
...
The `type`, `x`, `y` and `text` properties are required to create a text element, rest all attributes are optional
...
The `type`, `x`, and `y` properties are required, rest all attributes are optional
...
In addition to `type`, `x` and `y` properties, [`label`](https://github.com/excalidraw/excalidraw/blob/master/src/data/transform.ts#L124C7-L130C59) property is required for text containers. The `text` property in `label` is required, rest all are optional.
...
To bind arrow to a shape you need to specify its [`start`](https://github.com/excalidraw/excalidraw/blob/master/src/data/transform.ts#L86) and [`end`](https://github.com/excalidraw/excalidraw/blob/master/src/data/transform.ts#L54) properties. You need to pass either `type` or `id` property in `start` and `end` properties, rest all are optional.
