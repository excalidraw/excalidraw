...
For this purpose we introduced a new type [`ExcalidrawElementSkeleton`](https://github.com/excalidraw/excalidraw/blob/master/packages/element/src/transform.ts#L175). This is the simplified version of [`ExcalidrawElement`](https://github.com/excalidraw/excalidraw/blob/master/packages/element/src/types.ts#L206) type with the minimum possible attributes so that creating elements programmatically is much easier (especially for cases like binding arrows or creating text containers).

The [`ExcalidrawElementSkeleton`](https://github.com/excalidraw/excalidraw/blob/master/packages/element/src/transform.ts#L175) can be converted to fully qualified Excalidraw elements by using [`convertToExcalidrawElements`](https://github.com/excalidraw/excalidraw/blob/master/packages/element/src/transform.ts#L509).
...
You can pass additional [`properties`](https://github.com/excalidraw/excalidraw/blob/master/packages/element/src/types.ts#L40) as well to decorate the shapes.
...
In addition to `type`, `x` and `y` properties, [`label`](https://github.com/excalidraw/excalidraw/blob/master/packages/element/src/transform.ts#L166) property is required for text containers. The `text` property in `label` is required, rest of the attributes are optional.
...
A sticky note is an always-filled note whose label auto-fits: the font shrinks from the label's `fontSize` (the note's font ceiling) down to a minimum, and only then does the note grow in height. Only `type`, `x` and `y` are required; `width`/`height` default to a 250×250 square and `label` is optional. The note's `strokeColor` is its ink — its text and creation-date footer are always one color; a label that sets its own `strokeColor` gives it to the note. A footer band below the label shows the note's creation date (`created`); pass `created: null` for a note without one (the band stays reserved, but empty).
...
Notes keep their invariants regardless of the properties you pass: the background is never transparent (it falls back to the default yellow), the fill is solid, and the size is at least 75×75. The height you pass is kept as the note's `baseHeight`; if a label needs more room at the minimum font size, the created note is taller.
...
To bind arrow to a shape you need to specify its [`start`](https://github.com/excalidraw/excalidraw/blob/master/packages/element/src/transform.ts#L118) and [`end`](https://github.com/excalidraw/excalidraw/blob/master/packages/element/src/transform.ts#L76) properties. You need to pass either `type` or `id` property in `start` and `end` properties, rest of the attributes are optional
