...
```jsx live
function App() {
  const [bindaryData, setBindaryData] = useState(
    JSON.parse(localStorage.getItem("binaryData") || "[]")
  );
  const [elementData, setElementData] = useState(
    JSON.parse(localStorage.getItem("elementData") || "[]")
  );
...
const getExport = async () => {
    ...
    // Use spread operator to maintain the previous canvasUrl values
  ...
  return  (
    <>
      <button onClick={getExport}
        className="custom-button">
        Export to Canvas
      </button>
      <div className="export export-canvas">
        <img src={canvasUrl} alt="canvasUrl" />
      </div>
      ...
      <Excalidraw
          ...
          onChange={(elements: any, b: any, binary: any) => {
            localStorage.setItem("binaryData", JSON.stringify(binary));
            localStorage.setItem("elementData", JSON.stringify(elements));
            setBindaryData(binary);
