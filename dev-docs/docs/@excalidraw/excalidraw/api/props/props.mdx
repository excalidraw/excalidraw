...
## Mermaid export in the UI

Excalidraw currently includes a built-in UI action to copy supported diagrams as Mermaid flowcharts.

- In the app UI, this is exposed through the context menu and the command palette.
- In custom integrations, you can render the menu item via [`MainMenu.DefaultItems.CopyAsMermaid`](/docs/@excalidraw/excalidraw/api/children-components/main-menu#mainmenudefaultitemscopyasmermaid).
- There is currently no dedicated prop to configure Mermaid export behaviour and no dedicated `excalidrawAPI` method for it.
