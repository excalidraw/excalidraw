...
| [`iframeURLWhitelist`](#iframeURLWhitelist) | `(RegExp | boolean)[]` | _ | Set this prop to extend the list of allowed sites to be included in iFrames |
...
This is an optional property. Default sites allowed by Excalidraw include YouTube, Vimeo and Figma. You may allow additional sites to be included in iFrames. If you pass `[true]` all URLs will be allowed, if you pass `[...,false]`, then the rest of the validators after `false` will not be executed, including the omission of the excalidraw default validators. If you only send `(RegExp | boolean)[]` then after running the validators Excalidraw will also execute the built in validators.

```tsx
iframeURLWhitelist?: (RegExp | boolean)[];
```