...
There is currently no dedicated `excalidrawAPI` method such as `exportToMermaid()`.

The built-in Mermaid export that ships in the Excalidraw app is currently exposed through UI actions such as the main menu item and command palette entry, not as a public programmatic API for embedders.

If you are embedding Excalidraw and need Mermaid output today, the pragmatic approach is:

- Read the current scene using `getSceneElements()`.
- Restrict export to a supported subset of elements.
- Serialize that subset to Mermaid in host code.

This keeps the API surface honest while the feature matures.
