...
| [setViewport](#setviewport) | `function` | Navigate the viewport to a target and optionally lock pan/zoom to it. |
...
Navigates the viewport to a target and, optionally, locks pan/zoom to it. The resolved target box drives both the navigation and the lock: the viewport first animates onto the target (per `fit`), then the lock is installed against the settled viewport. Passing `null` clears any active lock without navigating.

| Attribute | type | default | Description |
| --- | --- | --- | --- |
| target | `Bounds` &#124; `{ x; y; width?; height? }` &#124; `ExcalidrawElement` &#124; `ExcalidrawElement[]` &#124; `string` | — | What to show in the viewport: an explicit scene-coordinate box/rect, element(s), or an element id / element-link URL. |
| fit | `"scale-down"` &#124; `"contain"` &#124; `"none"` | `"scale-down"` | `scale-down` zooms out to fit the target, capped at 100%; `contain` scales the target to fill the viewport (may exceed 100%); `none` keeps the current zoom and only centers the target. |
| lock.scroll | boolean | false | Constrain panning so the viewport stays within the target box. |
| lock.zoom | boolean | false | Make the resolved zoom the minimum the user can zoom out to. Zooming in (up to 3000%) is always allowed. |
| lock.overscroll | boolean &#124; number | true | Rubberband give past the lock, in viewport pixels (zoom-independent): how far the user can pan beyond the constraint before it snaps back. `true` uses the default (200px), `false` or `0` makes the lock rigid. |
| animation | `{ duration?: number }` &#124; `boolean` | `{ duration: 250 }` | `true` (or omitted) animates with the default duration; `false` jumps instantly. |
| offsets | `{ top?; right?; bottom?; left? }` | — | CSS-style padding in viewport pixels (zoom-independent). Insets the fit and extends the locked scroll area. |
