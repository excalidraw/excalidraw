...
| [scrollTo](#scrollto) | `function` | Navigate the viewport to a target and optionally lock pan/zoom to it. |
...
(
  opts:
    | null
    | {
        target: Bounds | ExcalidrawElement | ExcalidrawElement[] | string;
        behavior: "panOnly" | "zoomToFit" | "zoomToTarget";
        lock?: {
          scroll?: boolean;
          zoom?: boolean;
          tolerance?: number;
...
Navigates the viewport to a target and, optionally, locks pan/zoom to it. The resolved target box drives both the navigation and the lock: the viewport first animates onto the target (per `behavior`), then the lock is installed against the settled viewport. Passing `null` clears any active lock without navigating.

| Attribute | type | default | Description |
| --- | --- | --- | --- |
| target | `Bounds` &#124; `ExcalidrawElement` &#124; `ExcalidrawElement[]` &#124; `string` | — | What to scroll to: an explicit scene-coordinate box, element(s), or an element id / element-link URL. |
| behavior | `"panOnly"` &#124; `"zoomToFit"` &#124; `"zoomToTarget"` | — | `panOnly` keeps the current zoom and only recenters; `zoomToFit` zooms out to fit the target, capped at 100%; `zoomToTarget` scales the target to fill the viewport (may exceed 100%). |
...
| lock.tolerance | number | 0 | Rubberband overscroll allowance, in viewport pixels (zoom-independent). |
| animation | `{ duration?: number }` &#124; `boolean` | `{ duration: 250 }` | `true` (or omitted) animates with the default duration; `false` jumps instantly. |
| offset | `{ top?; right?; bottom?; left? }` | — | CSS-style padding in viewport pixels (zoom-independent). Insets the fit and extends the locked scroll area. |
