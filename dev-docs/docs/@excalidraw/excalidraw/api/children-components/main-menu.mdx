...
Renders the built-in action that copies the current selection, or the whole scene when nothing is selected, as Mermaid `flowchart` syntax.

The current implementation is intentionally scoped:

- Supports rectangles, diamonds, ellipses, standalone text, and arrows bound between supported nodes.
- Uses bound text as Mermaid node/edge labels when present.
- Fails instead of producing invalid Mermaid when the scene contains unsupported elements such as images, frames, embeds, or unbound arrows.
