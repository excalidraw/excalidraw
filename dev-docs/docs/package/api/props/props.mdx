...
| --- | --- | --- | --- |
| [`initialData`](/docs/package/api/props/initialdata) |  `object` &#124; `null` &#124; <code>Promise<object &#124; null></code> | `null` | The initial data with which app loads. |
...
| [`renderTopRightUI`](/docs/package/api/props/render-props#rendertoprightui) | `function` | _ | Render function that renders custom UI in top right corner |
| [`renderCustomStats`](/docs/package/api/props/render-props#rendercustomstats) | `function` | _ | Render function that can be used to render custom stats on the stats dialog. |
| [`renderSidebar`](/docs/package/api/props/render-props#rendersidebar) | `function` | _ | Render function that renders custom sidebar. |
...
| [`UIOptions`](/docs/package/api/props/ui-options) | `object` | [DEFAULT UI OPTIONS](https://github.com/excalidraw/excalidraw/blob/master/src/constants.ts#L151) | To customise UI options. Currently we support customising [`canvas actions`](#canvasactions) |
...
You can add `customData` to elements when passing them as [`initialData`](/docs/package/api/props/initialdata), or using [`updateScene`](/docs/package/api/props/ref#updatescene) / [`updateLibrary`](/docs/package/api/props/ref#updatelibrary) afterwards.
...
Defaults to *window.location.origin + window.location.pathname*. To install the libraries in the same tab from which it was opened, you need to set <highlight>window.name</highlight> (to any alphanumeric string) — if it's not set it will open in a new tab.
