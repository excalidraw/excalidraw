...
//     // Function to handle when text editing is completed via WYSIWYG
//     const handleTextUpdate = (element: RabbitSearchBoxElement, newText: string) => {
//       // Update both text and currentText to keep them in sync
...
//     // Listen for scene updates to catch text changes
//     const handleSceneUpdate = () => {
//       const elements = excalidrawAPI.getSceneElements();
      
//       elements.forEach(element => {
//         if (isRabbitSearchBoxElement(element)) {
//           // If text was updated via WYSIWYG, sync the properties
//           if (element.originalText && element.originalText !== element.currentText) {
//             const updatedElement = handleTextUpdate(element, element.originalText);
            
//             // Update the scene with synced properties
//             excalidrawAPI.updateScene({
//               elements: elements.map(el => 
...
//     // Register minimal event handling (remove custom text editing)
//     const intervalId = setInterval(handleSceneUpdate, 100);
