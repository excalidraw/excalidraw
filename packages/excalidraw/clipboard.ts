...
const parsePotentialSpreadsheet = (text: string): { spreadsheet: Spreadsheet } | { errorMessage: string } | null => {
...
const maybeParseHTMLPaste = (event: ClipboardEvent): { type: "mixedContent"; value: PastedMixedContent } | null => {
...
export const parseClipboard = async (event: ClipboardEvent, isPlainPaste = false): Promise<ClipboardData> => {
  ...
  try {
    // Check for JSON data containing scene information
    const jsonData = event.clipboardData?.getData(MIME_TYPES.json)
    if (jsonData) {
      try {
        const parsedJson = JSON.parse(jsonData)
        if (parsedJson.type === EXPORT_DATA_TYPES.excalidrawClipboard && parsedJson.sceneData) {
          return {
            elements: parsedJson.sceneData.elements || [],
            files: parsedJson.sceneData.files,
            sceneData: parsedJson.sceneData,
            programmaticAPI: true,
          }
        }
      } catch (jsonError) {
        console.warn("Failed to parse JSON data from clipboard:", jsonError)
      }
    }

    // Check localStorage for scene data (fallback)
    const storedSceneData = localStorage.getItem("excalidraw-clipboard-scene")
    if (storedSceneData) {
      try {
        const parsedSceneData = JSON.parse(storedSceneData)
        // Only use stored data if it's recent (within last minute)
        if (parsedSceneData.timestamp && Date.now() - parsedSceneData.timestamp < 60000) {
          return {
            elements: parsedSceneData.elements || [],
            files: parsedSceneData.files,
            sceneData: parsedSceneData,
            programmaticAPI: true,
          }
        }
      } catch (storageError) {
        console.warn("Failed to parse stored scene data:", storageError)
      }
    }
...
export const copyBlobToClipboardAsPng = async (blob: Blob | Promise<Blob>, sceneData = null) => {
  try {
    // Store scene data in localStorage if provided
    if (sceneData) {
      try {
        const sceneDataWithTimestamp = {
          ...sceneData,
          timestamp: Date.now(),
        }
        localStorage.setItem("excalidraw-clipboard-scene", JSON.stringify(sceneDataWithTimestamp))
      } catch (error) {
        console.error("Failed to store scene data in localStorage:", error)
      }
    }

    // Fix for the spread type error - create a proper object to spread
    const jsonData = sceneData
      ? {
          [MIME_TYPES.json]: new Blob([JSON.stringify({ type: EXPORT_DATA_TYPES.excalidrawClipboard, sceneData })], {
    ...
    if (isPromiseLike(blob)) {
      ...
      // Create a proper object to spread for the resolved blob case
      const jsonData = sceneData
        ? {
            [MIME_TYPES.json]: new Blob([JSON.stringify({ type: EXPORT_DATA_TYPES.excalidrawClipboard, sceneData })], {
              type: MIME_TYPES.json,
      ...
      await navigator.clipboard.write([
        new window.ClipboardItem({
          [MIME_TYPES.png]: resolvedBlob,
          ...jsonData,
        }),
      ])
    } else {
      // If including the scene data is causing the error, try again with just the image
      if (sceneData) {
        try {
          await navigator.clipboard.write([
            new window.ClipboardItem({
          ...
          console.warn("Failed to include scene data in clipboard, but image was copied")
          return
        } catch (fallbackError) {
          throw fallbackError
        }
...
// Add a convenience function to copy both image and scene data
export const copyImageWithSceneDataToClipboard = async (imageBlob: Blob | Promise<Blob>, sceneData: any) => {
  return copyBlobToClipboardAsPng(imageBlob, sceneData)
}

export const copyTextToSystemClipboard = async (text: string | null, clipboardEvent?: ClipboardEvent | null) => {
