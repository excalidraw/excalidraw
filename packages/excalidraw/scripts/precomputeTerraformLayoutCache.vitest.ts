...
describe.runIf(process.env.PRECOMPUTE_LAYOUT_CACHE === "1")(
  ...
  () => {
    it(
      "writes wrangler kv bulk put payload for catalog presets",
      async () => {
        const version = (process.env.LAYOUT_CACHE_VERSION ?? "").trim().slice(0, 12);
        ...
        for (const presetId of presetIds) {
          ...
          for (const view of views) {
            ...
            for (const pack of packs) {
              ...
              if (!result.ok) {
                throw new Error(
                  `Layout failed for ${presetId}/${view}${pack ? `/${pack}` : ""}: ${result.error}`,
                );
              }
