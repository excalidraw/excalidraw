...
class App extends React.Component<AppProps, AppState> {
  ...
  private handleCanvasDoubleClick = (
    ...
    // Add your check here for non-editable elements
    if (selectedElements.length === 1) {
      const element = selectedElements[0];

      // Define which elements should not be text-editable
      if (
        element.type === "rabbit-image" ||
        // element.type === "rabbit-searchbox" ||
        element.type === "rabbit-image-tabs" ||
        element.type === "rabbit-color-palette" 
      ) {
        return; // Exit early, don't start text editing
      }
    }
