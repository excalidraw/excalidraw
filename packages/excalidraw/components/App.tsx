...
class App extends React.Component<AppProps, AppState> {
  ...
  public onExportImage = async (
    ...
    if (type === "png" || type === "svg" || type === "clipboard") {
      ...
      try {
        ...
        const canvas = await exportToCanvas({
          ...
          appState: {
            ...this.state,
            exportBackground: this.state.exportBackground,
            exportWithDarkMode: this.state.exportWithDarkMode,
            exportScale: this.state.exportScale,
            exportEmbedScene: this.state.exportEmbedScene,
            name: this.getName(),
            viewBackgroundColor: this.state.viewBackgroundColor,
          },
          files: this.files,
        ...
        // --------------------------------------------------------------
        // 2. Convert canvas → JPEG Blob (92 % quality)
        // --------------------------------------------------------------
        ...
        // --------------------------------------------------------------
        // 3. Prompt user for a file name
        // --------------------------------------------------------------
        const defaultName = `${this.getName() || "drawing"}.jpg`;
        const userName = window.prompt(
          "Enter file name for the JPEG:",
          defaultName,
        );
    
        // If the user cancels → abort the download
        if (userName === null) {
          // Silently exit – no download, no error
          return;
        }
    
        // Sanitize the name (remove extension, re-add .jpg)
        const sanitizedName = userName
          .replace(/\.jpe?g$/i, "")   // strip any existing .jpg/.jpeg
          .trim()
          || "drawing";               // fallback if empty
    
        const fileName = `${sanitizedName}.jpg`;
    
        // --------------------------------------------------------------
        // 4. Trigger the download with the chosen name
        // --------------------------------------------------------------
        ...
        // --------------------------------------------------------------
        // 5. (Optional) Fake a FileSystemFileHandle for embed-scene
        // --------------------------------------------------------------
        if (this.state.exportEmbedScene) {
          ...
          } satisfies Partial<FileSystemFileHandle>;
    
          // @ts-ignore – we only need the shape used by isImageFileHandle
          if (isImageFileHandle(fakeHandle)) {
            this.setState({ fileHandle: fakeHandle as any });
          }
        }
