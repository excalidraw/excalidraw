...
import { logCoverage } from './tests/coverageTool';
...
export const orderedColinearOrientation = (p: Point, q: Point, r: Point) => {
  const val = (q[1] - p[1]) * (r[0] - q[0]) - (q[0] - p[0]) * (r[1] - q[1]);

  if (val === 0) {
    logCoverage('orderedColinearOrientation', 'colinearBranch')
    return 0;
  }
  return val > 0 ? 
  (logCoverage('orderedColinearOrientation', 'clockwiseBranch'), 1) :
  (logCoverage('orderedColinearOrientation', 'counterclockwiseBranch'), 2);
};

// Check is p1q1 intersects with p2q2
export const doSegmentsIntersect = (p1: Point, q1: Point, p2: Point, q2: Point) => {
  ...
  if (o1 !== o2 && o3 !== o4) {
    logCoverage('doSegmentsIntersect', 'generalCaseBranch');
    return true;
  }

  if (o1 === 0 && isPointWithinBounds(p1, p2, q1)) {
    logCoverage('doSegmentsIntersect', 'specCase1Branch');
    return true;
  }

  if (o2 === 0 && isPointWithinBounds(p1, q2, q1)) {
    logCoverage('doSegmentsIntersect', 'specCase2Branch');
    return true;
  }

  if (o3 === 0 && isPointWithinBounds(p2, p1, q2)) {
    logCoverage('doSegmentsIntersect', 'specCase3Branch');
    return true;
  }

  if (o4 === 0 && isPointWithinBounds(p2, q1, q2)) {
    logCoverage('doSegmentsIntersect', 'specCase4Branch');
    return true;
  }

  logCoverage('doSegmentsIntersect', 'notIntersectingBranch');
  return false;
};
