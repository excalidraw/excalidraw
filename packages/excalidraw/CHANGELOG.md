...
- Screen reader / non-visual accessibility layer (WCAG 2.2 AA): an invisible, spatially-positioned scene mirror exposes every canvas element to assistive technology with derived names ("Start", rectangle, 3 of 12, connected to "End"…); Tab / Ctrl+Alt+Arrows browse elements (auto-panning, focus-visible ring), Space builds multi-selections, Enter places default-sized elements below the browsed anchor, Alt+Shift+Arrows/R/E resize & rotate, Alt+C connects two shapes with a bound arrow, and line points / image crops are keyboard-editable; a singleton live region announces tool, selection, move/resize/rotate, undo/redo, delete/duplicate/group/align/flip, zoom, hints and collaborator join/leave; "How to navigate with a screen reader" guide dialog on Alt+Shift+H; F6 cycles editor regions and Alt+Shift+A jumps straight to the canvas; single-key tool shortcuts can be disabled (WCAG 2.1.4); image alt text (persisted in `customData.a11y.altText`) and per-element `<title>`s in SVG exports; element names include conceptual colors ("red", "blue, light green fill" — never hex codes) so same-colored elements can be recognized as belonging together, stroke/fill changes are announced, and color-picker swatches are named by concept; dialog/menu/palette/tooltip/library/stats/search chrome got correct roles, names and keyboard support. Pointer UX and visuals are unchanged. [#TBD](https://github.com/excalidraw/excalidraw/pull/TBD)
