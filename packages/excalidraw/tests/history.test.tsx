...
import { Snapshot, CaptureIncrementAction } from "../store";
...
describe("history", () => {
  ...
  describe("singleplayer undo/redo", () => {
    ...
    it("should not end up with history entry when there are no elements changes", async () => {
      ...
      API.updateScene({
        elements: [rect1, rect2],
        captureIncrement: CaptureIncrementAction.IMMEDIATELY, // even though the flag is on, same elements are passed, nothing to commit
      });
      expect(API.getUndoStack().length).toBe(1);
      expect(API.getRedoStack().length).toBe(0);
      expect(h.elements).toEqual([
