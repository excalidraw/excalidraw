...
공백 줄바꿈\n전각기호…—`);
  ...
  describe("wrapTextPreservingWhitespaceWithExplicitNewlineMarkers", () => {
    it("should not mark soft wraps as explicit newlines", () => {
      ...
      expect(explicitNewlineAfterLine).toHaveLength(lines.length);
      expect(explicitNewlineAfterLine.some(Boolean)).toBe(false);
    });

    it("should mark only real newlines even when wrapping happens", () => {
      const firstLine = wrapTextPreservingWhitespaceWithExplicitNewlineMarkers(
        "Hello World",
        font,
        50,
      );
      ...
      expect(lines.length).toBeGreaterThan(firstLine.lines.length);
      expect(explicitNewlineAfterLine).toHaveLength(lines.length);

      // 原文只有一个真实换行符，因此数组里应当仅有一个 true
      expect(explicitNewlineAfterLine.filter(Boolean)).toHaveLength(1);

      // 真实换行符应当标记在“第一段（Hello World）最后一行”的末尾
      const explicitBreakIndex = firstLine.lines.length - 1;
      expect(explicitNewlineAfterLine[explicitBreakIndex]).toBe(true);
    });

    it("should preserve consecutive explicit newlines", () => {
      const { lines, explicitNewlineAfterLine } =
        wrapTextPreservingWhitespaceWithExplicitNewlineMarkers(
          "a\n\nb",
          font,
      ...
      expect(explicitNewlineAfterLine).toEqual([true, true, false]);
    });
  });
});

describe("Test wrapTextPreservingWhitespace", () => {
  const font = "10px Cascadia, Segoe UI Emoji" as FontString;

  it("should return the text as is if max width is invalid", () => {
    const text = "Hello Excalidraw";
    expect(wrapTextPreservingWhitespace(text, font, NaN)).toEqual(text);
    expect(wrapTextPreservingWhitespace(text, font, -1)).toEqual(text);
    expect(wrapTextPreservingWhitespace(text, font, Infinity)).toEqual(text);
  });

  it("should preserve all non-newline characters when wrapping", () => {
    const text = "Hello     world!";
    const res = wrapTextPreservingWhitespace(text, font, 60);
    expect(res.split("\n").join("")).toBe(text.split("\n").join(""));
  });

  it("should preserve trailing whitespaces when wrapping", () => {
    const text = "Hello     ";
    const res = wrapTextPreservingWhitespace(text, font, 30);
    expect(res.split("\n").join("")).toBe(text.split("\n").join(""));
  });
});
