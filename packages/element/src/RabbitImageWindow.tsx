...
export const RabbitImageWindow: React.FC<RabbitImageWindowProps> = ({
    ...
    // Get aspect ratio based on tab type
    const getAspectRatio = () => {
        const currentTabName = tabData[activeTab]?.name?.toLowerCase();
        if (currentTabName === "youtube") {
            return "16 / 9"; // 16:9 aspect ratio for YouTube
        } else if (currentTabName === "internet webpages") {
            return "auto"; // No fixed aspect ratio for webpages
        } else {
            return "1"; // Square aspect ratio for other tabs (Google, Pinterest)
        }
    };
    ...
    return (
        <>
            {/* Rabbit GIF positioned relative to the window */}
            <img
                src="https://i.imgur.com/P1tu0MX.gif"
                alt="Rabbit"
                style={{
                    position: "absolute",
                    top: pos.y - 50,
