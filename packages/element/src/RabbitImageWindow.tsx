...
//     const [isMinimized, setIsMinimized] = useState(false);
//     const dragRef = useRef<{ x: number; y: number } | null>(null);
//     const resizeRef = useRef<{ x: number; y: number; startWidth: number; startHeight: number } | null>(null);
//     const [hoveredCard, setHoveredCard] = useState<string | null>(null);
...
//         setSize((prev) => {
//             const newWidth = Math.max(MIN_WIDTH, Math.min(MAX_WIDTH, resizeRef.current!.startWidth + dx));
//             const newHeight = Math.max(MIN_HEIGHT, Math.min(MAX_HEIGHT, resizeRef.current!.startHeight + dy));
