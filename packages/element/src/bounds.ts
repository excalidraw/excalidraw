...
export class ElementBounds {
  ...
  private static calculateBounds(
    ...
    if (isFreeDrawElement(element)) {
      ...
      const rotatedPoints = getStarPoints(element).map(([px, py]) =>
        pointRotateRads(
...
/** Returns local (unrotated) vertices of a 5-point star inside the element bbox. */
export const getStarPoints = (
  element: ExcalidrawElement,
): [number, number][] => {
  const cx = element.width / 2;
  const cy = element.height / 2;
  const outerRx = Math.max(cx, 1);
  const outerRy = Math.max(cy, 1);
  const innerRx = outerRx * 0.382;
  const innerRy = outerRy * 0.382;

  const points: [number, number][] = [];
  for (let i = 0; i < 10; i++) {
    const radiusX = i % 2 === 0 ? outerRx : innerRx;
    const radiusY = i % 2 === 0 ? outerRy : innerRy;
