#!/usr/bin/env node
/**
 * Patch LocalStack terraform show -json output so pipeline layout can resolve
 * account/region/VPC placement (create plans omit subnet ARNs and provider locals).
 */
...
function subnetNameTag(stack) {
  if (stack.kind === "consumer") {
    return "local-consumer-private";
  }
...
function enrichResourceChange(rc, stack, stateValues) {
  ...
  if (
    ...
    const functionName =
      ...
      (stack.kind === "consumer"
        ...
        : `local-${stack.apiName}`);
    const subnetId = resolveSubnetId(stateValues, stack);
    const lambdaSubnetId =
      state.vpc_config?.[0]?.subnet_ids?.[0] ?? subnetId;
    if (
      state.vpc_config?.[0]?.subnet_ids?.[0] &&
      subnetId &&
      lambdaSubnetId !== subnetId
    ) {
      console.warn(
        `[${stack.id}] lambda subnet id ${lambdaSubnetId} != network subnet ${subnetId}; using network subnet`,
      );
    }
    mergeAfter(rc.change, {
      arn:
  ...
  if (
    ...
    rc.address === "module.api.aws_lambda_permission.apigw"
  ) {
    mergeAfter(rc.change, {
      function_name:
        state.function_name ??
        stateValues.get("module.api.aws_lambda_function.this")?.function_name ??
        `local-${stack.apiName}`,
      region,
    });
  }
