...
provider "aws" {
  ...
  assume_role {
    ...
    session_name = "terraform-staging-west-api-10"
  }
...
module "api" {
  ...
  artifact_bucket_id    = data.terraform_remote_state.west_network.outputs.lambda_artifacts_bucket_id
  lambda_source_file    = "${path.module}/../shared/api_handler.py"
  openapi_template_path = "${path.module}/openapi.tftpl"
  stage_name            = "v1"
  dynamodb_table_arn    = data.terraform_remote_state.west_datastores.outputs.api10_table_arn
  tags                  = local.tags
}
