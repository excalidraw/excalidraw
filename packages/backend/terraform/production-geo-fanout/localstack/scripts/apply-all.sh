...
echo "Starting LocalStack containers (skip if already healthy)..."
if curl -sf "http://localhost:4566/_localstack/health" >/dev/null && curl -sf "http://localhost:4567/_localstack/health" >/dev/null; then
  echo "LocalStack already running on 4566/4567 — reusing existing containers."
else
...
echo "All production-geo-fanout LocalStack stacks applied."
