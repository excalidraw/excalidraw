...
**Swimlane policy semantics (cyclic container, [DEC-8(C)](#14-open-design-decisions)).** When a container's hull graph `D_H` is **cyclic**, the forced/packed split is replaced: the container's whole subtree is dissolved onto **one shared cluster column axis** (column = dense rank of the cluster's global `LB` floor, so a TFD edge always crosses a column — CON-1/CON-12), and each intermediate hull (subnet zone / VPC) becomes a **Y-lane** spanning the column range of its own clusters. A subnet is therefore one contiguous frame **over multiple columns**, and cross-subnet dataflow `A→B→A` reads as forward column steps across vertically-separated lanes — no edge backward, none same-column. This is the **Sander [R6]** compound model ("clusters span the contiguous level range of their children; hulls derived"); the spurious sibling-hull cycle (an acyclic `D` up-projects to a cyclic `D_H`, [§26](#26-edge-cases--degenerate-inputs)) is the trigger. Implemented in `arrangeLaneSubtree` (placement); the outermost cyclic ancestor owns the axis.
...
- **Lane (cyclic interior, [DEC-8(C)](#14-open-design-decisions)):** when an interior container's `D_H` is **cyclic** (a spurious hull cycle, [§26](#26-edge-cases--degenerate-inputs)), the local-column boundary for X moves **up** from `subnetZone` to that container (typically the VPC): all of its descendant clusters share **one** container-local column axis, so the subnet hulls read L→R by dependency instead of each owning a single column. Still **local** (container-scoped), not global — REQ-7's hybrid model holds; the axis is no wider than the count of distinct `LB` depths present (bounds the §3.4 width cost).
...
| **Iron rule (no backward AND no same-column edge)** | `acyclicBackwardEdges = 0` **and** `acyclicSameColumnEdges = 0` (Compact **and** Full) — measured on placed **boxes** (`backwardEdgeGate`, on the box **left edge**), so it does not go blind when frames lack `terraformPrimaryAddress` in Full. `cyclicBackwardEdges` / `cyclicSameColumnEdges` counted + **excused** (genuine `D`-cycle wrap-edges, drawn via EXT-12) — both **0 on v2** (no genuine `D` cycle). | CON-12 |
| **Determinism** | Two builds byte-identical. | CON-8 |
| **Acyclic guard** | Cluster graph cycles localized (CON-2); a **spurious** per-container `D_H` cycle is dissolved into a swimlane (DEC-8(C)) so its interior reads strictly L→R. | CON-1/2 |
