...
### 8.1 RCLL ancillary ("All resources") — reserved band (BUILT)

> **Status: BUILT, baseline reserved-band path** (2026-06-22; DI-ANC-1..4, [§34.2](#342-implemented-vs-specified-delta-as-built-m3a-hardening)). RCLL now honors the **All resources** toggle: unconnected resources render as per-scope "Unconnected" strips in first-class `ancillaryBand` leaves. The bands reserve bottom-row space in their owning hull, add height only when rows are inserted, and are excluded from dataflow leaf/model accounting.
...
**Allocator status.** The outside-in width allocator is not yet spending sibling/root slack; the current build uses each host hull's baseline content width and stamps deterministic allocator metrics with zero allocations (`widenedHullCount`, `allocatedWidthPx`, `rowSavings`, `fallbackDropCount`). This preserves the dataflow-first baseline and leaves the row-saving allocator as a follow-up over the now-built reserved-band primitive.
...
| **Ancillary strips** ("All resources") | the unconnected resources render as per-scope "Unconnected" strips (compound/classic behavior) | **Drawn in RCLL via baseline post-placement reserved bands.** Earlier export-phase placement measured 90/86 collisions on v2; the built path instead inserts bottom-band rows into the already-placed Dataflow tree, excludes bands from dataflow collectors/model primary counts, and parents strips under provider/account/region/VPC hulls. | Outside-in width allocation is not yet active; allocator metrics report zero allocations in the baseline implementation. The invariant is Dataflow X/shape unchanged; only downward Y growth is allowed. |
...
| `uncommitted` | **Ancillary** | **RCLL "All resources" reserved bands.** `ancillaryBand` placement role; Dataflow-only baseline placement plus post-placement band insertion; provider/account/region/VPC ancillary scopes; strip rendering + frame parenting via `ancillaryScopeRole`; UI re-enabled; meta counts + allocator metrics; v2 regression locks no widening, no collisions, and primary X/shape preservation. | DI-ANC-4 (baseline reserved-band primitive built; outside-in width allocator deferred as a zero-allocation metric surface). | Supersedes DI-ANC-1/3 disabled/deferred state; updates §8.1 and §34.2. |
