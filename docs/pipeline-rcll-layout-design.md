...
| **CON-12** | **The iron rule — no backward edge.** No collapsed TFD edge `u→v` renders backward (`centerX(box(v)) ≥ centerX(box(u))`), **except** within a **cyclic** container, where a cycle has no consistent L→R order and the wrap-edge is drawn as an explicit back-edge ([EXT-12](#23-human-factors-readability--principles-engine-practice--optional-extras)). Implied by [CON-1](#4-requirements-catalogue) + the **width-aware** staircase ([CON-6](#4-requirements-catalogue) restated below) for every acyclic LCA; cyclic LCAs are excused (parallel to CON-1/CON-2). | User (hard rule); width-aware staircase makes local columns globally-monotonic along every edge. | User | `backwardEdgeGate` on placed boxes ([§13](#13-invariants--acceptance-gates)): `acyclicBackwardEdges = 0` (Compact **and** Full); `cyclicBackwardEdges` counted + excused. |
...
| **Iron rule (no backward edge)** | `acyclicBackwardEdges = 0` (Compact **and** Full) — measured on placed **boxes** (`backwardEdgeGate`), so it does not go blind when frames lack `terraformPrimaryAddress` in Full. `cyclicBackwardEdges` is counted + **excused** (cycle wrap-edges, drawn via EXT-12). | CON-12 |
| **Determinism** | Two builds byte-identical. | CON-8 |
| **Acyclic guard** | Cluster graph cycles localized (CON-2); the per-container `D_H` cycles are condensed (DEC-8(B)). | CON-1/2 |
...
- **Status: (B) IMPLEMENTED (M3a-hardening, DI-M3a-12).** `columnsForContainer` condenses each cyclic container's strongly-connected components (iterative Tarjan) and runs longest-path on the **condensation DAG**: every member of one SCC gets the **same** `localColumn` (M3a packed placement stacks them in Y at one shared X — so the cycle's wrap-edge does not render backward), while the container's *acyclic* members keep distinct forward columns. This supersedes M2's earlier "sequential columns `0,1,2…`" strip (which spread cycle members across X). On v2 this leaves only the irreducible **intra-SCC wrap-edges** (11 compact / 22 full) backward — a cycle of width-bearing members cannot be drawn fully L→R; those are excused by CON-12 and slated for EXT-12 back-edge styling. Option (A) (feedback-arc-set, one styled back-edge per cycle) remains the path to mark each cycle with a *single* explicit back-edge if desired.
...
| **DI-M3a-12** | M3a-hardening | **Cyclic containers → DEC-8(B) SCC condensation** (iterative Tarjan + longest-path on the condensation): SCC members share one `localColumn`; acyclic members stay forward. | Replaces M2's sequential-column strip, which spread cycle members across X and caused ~half the backward reads. A shared column ⇒ M3a packed stacks members in Y at one X ⇒ no intra-SCC backward render beyond the irreducible wrap-edge. **Supersedes DI-M2-2** (cyclic → sequential columns). | no (core) |
| **DI-M3a-13** | M3a-hardening | The residual intra-SCC wrap-edges (11 compact / 22 full on v2) are **excused + styled as explicit back-edges (EXT-12 IMPLEMENTED)**: `styleRcllBackEdges` marks any TFD arrow whose target box reads left of its source (always an intra-cycle edge, given the CON-12 gate) as **dashed + distinct colour (`#e8590c`) + `terraformBackEdge`**; count in `meta.gates.backEdgesStyled`. Feedback-arc-set (DEC-8 option A, one designated back-edge per cycle) remains the alternative if the count should drop to one-per-cycle. | A cycle of width-bearing members has no fully-forward drawing (the geometric floor under DEC-8(B)); marking the wrap-edge makes it read as an intentional cycle, not a layout error. | option A revisit |
...
| **Cyclic placement** ([§26](#26-edge-cases--degenerate-inputs)/[DEC-8](#14-open-design-decisions)) | "model-order stack in a shared column band" | **DEC-8(B) via SCC condensation** — cycle members share one column; acyclic members stay forward (DI-M3a-12). | Implemented at M3a-hardening; the earlier sequential-strip is superseded. |
...
| **Ancillary strips** | — | NOT drawn in RCLL (model has no ancillary; view doesn't enable it). | Documented M3a limitation. |
...
| _(this commit)_ | docs | §34.3 commit-map hash backfill. | — | Filled `ee0e44e93` into the M3a-hardening row above (its own hash was unknowable when written). |
