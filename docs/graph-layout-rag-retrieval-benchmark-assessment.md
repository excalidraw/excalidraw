...
Do not enable local reranking by default. Use `--no-hybrid` only for deliberate
dense-only comparisons.

## Measured Results

Benchmark:
`tools/graph-layout-rag/data/eval/runs/current-index-pareto-20260613/benchmark.json`
...
RRF `k=100` scored `0.759` nDCG@10 versus `0.758` for the default `k=60`.
That difference is too small to treat as meaningful, so the simpler existing
`k=60` configuration remains the default.
...
- The current latency and memory measurements are representative of this machine
  when ingest and other model-heavy workloads are not running.
...
- The current ranking of strategies will remain unchanged after expanding the
  corpus or gold set.
...
- Ten original PDF-only cases had no retrievable PDF label; the benchmark now
  separates catalog and PDF deep-read tracks to avoid impossible evaluation.
- Strategies were measured once, so confidence intervals and run-to-run latency
