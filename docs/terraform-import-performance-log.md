...
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| 2026-06-11 | **Packed pipeline layout (feature, opt-in, default off):** `pipelinePacked` now does real work — group-uniform rightward depth shifts for sink-only units (ALAP bounded by outgoing TFD edges) + hierarchical skyline Y re-packing in `terraformPipelineLayoutPacked.ts`. Flag-off path untouched. | pipeline | — | — | layout-neutral when off; packed adds two small passes on top of the classic grid | yes (flag off) | layout.pipeline | **Kept.** staging-extended-localstack-v2 height 18,522→13,867 px (−25%), width +24% (8,038→9,966). Packed scenes skip the KV preset layout cache (flag is not in the cache key yet). |
