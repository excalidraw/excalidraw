...
| History chat-switch / delete chat | **unguarded (N2)** | switch: generation **continues in the background** (single buffered handle — patches/`started` metadata route into the buffer, the canvas keeps painting; rejoin on return shows it live; terminal upserts + persists the origin chat's row via `saveBackgroundChat`) — _2026-07-19 post-QA direction, replaces the earlier auto-stop_; delete of the streaming chat (buffered or displayed) still auto-stops first |
...
2. C1/N2: send is a no-op while streaming; deleting the streaming chat auto-stops first; a chat **switch** mid-stream continues the generation in the background (single buffered handle, rejoined on switch-back; terminal persists the origin chat's row) — _amended 2026-07-19, post-QA direction; previously switch auto-stopped too_ (was: concurrent streams / orphaned stream corruption).
...
| 8 ✅ | **§4-QA fixes** (2026-07-19 post-QA direction): §5.8 re-run affordance; background continuation on chat switch (replaces N2 auto-stop-on-switch) | QA findings 1+2 | rewrite the N2 switch test; new re-run/background tests |
...
- ✅ **Step 8 landed 2026-07-19** (fable subagent, gates re-verified + live QA): both §4-QA findings fixed per user direction. **(1) Re-run affordance (§5.8)**: trailing prompt-only turns (reload mid-generation) render a "Run again" action on the user bubble (`ai.chat.actions.rerun`, RetryIcon, disabled at quota 0, hidden while any generation streams or once an assistant reply exists) → fresh turn via `sendChatPrompt(content, images)`; the server-side cancel of the orphaned in-flight generation is §7 item 7. **(2) Background continuation on chat switch** (replaces the N2 auto-stop-on-switch; delete of the streaming chat still stops): single-flight means at most one background generation, modeled as one buffer `{chatId, messages, updatedAt}` in `useTTAChatActions` — the lifecycle's `setChatMessages`/`applyServerChatMetadata` props are wrapped to route patches and `started` metadata into the buffer while its chat isn't displayed (`started`'s chatId can no longer touch `activeChatId` mid-switch); terminal-while-backgrounded persists the row via new `useTTAChatHistory.saveBackgroundChat` (the one non-active write; same `isPersistableMessage` filter + title derivation by construction) and clears the buffer; rejoin mid-stream adopts the buffer's messages live (no interrupted-normalization, no draft reset); the canvas keeps painting/committing throughout (scene-level, chat-agnostic). Edge decisions: retry-while-backgrounded stops+persists the backgrounded partial before replacing; buffered-chat delete stops + drops without persisting (no save-then-delete race); displayed-chat delete leaves the background stream running; never-`started` buffer dropped with a warn (same window as reload-before-`started`); Stop stays global (stops the backgrounded generation from any chat). §2.3 policy table + §5.2 amended. Tests 65→**74** (+2 known): N2 switch test rewritten to pin continuation+rejoin, + background-terminal-persist, started-routing, buffered/displayed delete matrix, re-run fresh-turn payload, 3 panel-level affordance tests, saveBackgroundChat unit test. Live QA: re-run on the prompt-only castle chat → fresh turn completed, affordance correctly hidden during/after; switch-away mid-stream → stream continued (global Stop visible in the other chat), **finished while away, rejoin hydrated the terminal bubble from the persisted row**; rejoin mid-stream shows the live streaming bubble; zero intermediate elements, no stuck state.

- ✅ **Full §4 feature-parity QA (2026-07-19, localhost:3005 + dev oss-ai-server; live via chrome-devtools MCP + a parallel code-verification pass over the non-simulatable items).** Live PASS: trigger/panel (footer pill + aria wiring, 400×720 bottom-right, hidden ≤768px, pinned default, unpinned closes on interactive-canvas pointerdown and on sidebar open, pinned shifts left of the open sidebar); composer (auto-grow to exactly 240px, Shift+Enter no-send, wheel inert over composer with canvas positive-control, paste→JPEG-1024 + content-hash dedupe + host `onMaxImages` `role="alert"` upsell (excalidraw.com maxImages=1 exercises the override path), preview modal via Enter + Escape swallowed, per-image remove, send⇄Stop swap); streaming (elapsed ticker, "completed in/failed after/stopped after" + stop-reason line); canvas (viewport-centered tagged inserts, single generation = exactly 1 undo entry, Stop commits with preview flags stripped + undo/redo clean, retry/regenerate replacement); messages (Retry on latest only, To canvas/Regenerate/Delete, C2 splice → interrupt error bubble + support banner + previous generation preserved, N1 retry payload omits `retryAssistantMessageId` on never-succeeded turn, retry success replaces the stopped partial — queue survived the failure); history (auto-focused substring search incl. prompt bodies, updatedAt desc, rename via Enter + maxLength 80, chat-delete confirm, N2 switch-mid-stream auto-stop + composer focus, two-tab hydration incl. renames); keyboard (Tab-steal, Cmd/Ctrl+Shift+O); reload-mid-stream (N3: zero intermediate elements after reload, no stuck bubble, prompt-only turn on the continue card). Code-verified (14 PASS): aiEnabled both gates, IME guard, no-drag&drop, quota-0 composer, idle-timer phases + label override, scroll-keeps-top, thumbnail-click insert, IO-fallback, host API surface + exports + X-Ratelimit parsing, non-features absent, ResizeObserver shift. **Parity note:** a follow-up generation produces 2 undo entries (previous-gen removal IMMEDIATELY at first render + final commit) — verified identical at pre-rewrite `3a7c9fbad`, so parity, but first-undo-shows-neither-generation is a pre-existing UX quirk worth a future look. **Findings:** (1) 🔴 the §2.5/§5.8 **re-run affordance for prompt-only trailing turns was never implemented** (reload/switch-away mid-stream leave the prompt with no button to run it again); (2) 🟡 **switch-away mid-stream drops the interrupted assistant bubble from persistence** (terminal stop patch lands after the debounced active-chat-gated save moves on — switch-back shows a prompt-only turn; canvas commit unaffected; fix: flush the pending save before flipping `activeChatId`, or fold it into the §5.8 affordance work since both paths now converge on prompt-only turns); (3) ⚪ `targetCenter` re-anchoring and final-insert selection are implemented but not test-pinned (from the code pass). Not live-simulated (code-verified only): quota-0 banner, offline sniff variants, empty-response label, h:mm:ss ticker.

- ✅ **Step 7 landed 2026-07-19** (fable subagent, gates re-verified): new `useTTAChatActions` (917L) owns all generation/chat orchestration — `sendChatPrompt`/`runGeneration`/`streamAssistantResponse`/`stopActiveGeneration`/`handleRetry`/`executeDelete`/`handleStartNewChat`/`handleSelectChat`/`handleDeleteChat`/`handleInsertResult`, with `useGenerationSlot` + `useAIStreamingLifecycle` composed inside; **TTADialog 1393→605 LOC** (target ~600 hit), keeping only UI state (open/pin, modal, history overlay, confirm-pending, composer, support banner, scroll/focus/keyboard effects, `useCanvasDraft` creation). `runGeneration` verified byte-identical to the step-2 landed version. History hook absorbed the six chat-id/updatedAt bookkeeping callbacks as two handlers: `applyServerChatMetadata({chatId?, updatedAt?})` + `touchActiveChatUpdatedAt()`. §8 polish: **M5** — both truncate catches now `app.api.setToast` (new `ai.chat.errors.deleteFailed` key; plain failure toast, not retryable — `AppState["toast"]` has no action affordance, accepted deviation); **§8.2** — history chat delete now confirms via the same ConfirmDialog pattern (`ai.chat.historyPanel.deletePrompt`); **§8.4** — rename input `maxLength={80}`. Deviations (sound): §2.7's "dialog keeps delete orchestration" contradicted the step scope — resolved per the step (executeDelete moved, dialog keeps confirm state only); `handleRenameChat` pass-through dropped; `sendChatPrompt` now memoized. The deferred dialog-level interaction tests landed as `useTTAChatActions.test.tsx` (5: C1 double-send no-op, N2 switch-mid-stream abort+commit+stopped+freed-slot, N1 omit-when-never-succeeded, N1 last-successful-attempt target, M5 toast-on-truncate-failure). **65 passing** + the 2 known, typecheck clean. The flagged Escape interplay was confirmed live and fixed in a follow-up commit: TTAHistory's window-capture Escape handler runs before any handler of a modal stacked on it, so it closed the overlay behind the chat-delete confirm — new `suspendEscape` prop on TTAHistory (fed by `view.isConfirmDialogOpen`) makes the overlay yield Escape while a confirm is pending. Smoke QA (2026-07-19, localhost:3005): send→complete→insert+select ✓, history hydration ✓, chat-delete confirm ✓ (Escape dismisses only the confirm; confirmed delete removes the row with history staying open; plain Escape still closes the overlay), rename `maxLength=80` ✓. Outstanding: the step-5/§4 full checklist items.

- ✅ **Step 6 landed 2026-07-19** (fable subagent, gates re-verified): `useAIAssistantPreview` rebuilt on one `useEffect` + epoch token (`activeRenderKeyRef` + `renderSeq` monotonic guard) + the shared `throttle.ts` — the 7-ref hand-rolled scheduler is gone (375→304 LOC; the §2.6 ~160 estimate didn't count the kept `renderAIAssistantPreviewDataUrl` + types + LRU, ~120 lines; the scheduler core did shrink as intended). **M7 fixed**: module cache is now a 32-entry LRU (Map insertion order = recency; hits/writes re-insert) and `evictAssistantPreviews(messageIds)` is called from `useTTAChatHistory.deleteChat` — the single choke point all three delete paths (history delete, active-chat delete, first-turn wipe) flow through. Parity kept: renderKey semantics (id+theme+streaming|complete, skeletons reference equality), `enabled: false` idle path, salvage rendering of errored partials, terminal renders immediate via cancel-then-call leading edge, streaming trailing flush unchanged. Sound deviations: a mid-stream render slower than the 300ms window is superseded latest-wins instead of parked-until-done (monotonicity guarantee); no unmount cancellation (at most one trailing render completes post-unmount and warms the cache — cleanup would drop the trailing render on every chunk); identity-preserving state updater stops the old fresh-object-per-effect-run churn. New `useAIAssistantPreview.test.tsx` (5: cache-hit-no-rerender, 32-cap LRU eviction incl. recency touch, evict helper, enabled:false, streaming throttle trailing flush); history delete test now asserts eviction. 60 passing + the 2 known, typecheck clean.

- ✅ **Fix: flashing streaming status labels (2026-07-19)**: root cause — the dev server streams continuous empty `partial` keep-alive frames (~8/s; 20-31 before content), and the per-chunk patch blindly downgraded the status to unlabeled `generating`, wiping "Thinking…"/server statusText ~0.65s after it appeared (the blind clear predates the rewrite — verified in `480d99783` and pre-rewrite `31d8e317a~1` — the union made it whole-object replaces but the flash is chunk-cadence-dependent, which is why earlier QA missed it). Fix in the lifecycle: guarded `patchStreamingStatus` — no-op patches keep the previous status object (identity-stable), empty keep-alive chunks use `keepLabel` (never downgrade a labeled state), `startedAt` carries over so the elapsed ticker never resets. Renderable chunks still legitimately clear the label. Live-verified pre/post at localhost:3005 (label survived 31 keep-alives post-fix; lived 0.65s pre-fix); pinned by a new lifecycle test that fails against the unfixed code.

- ✅ **Step 5 landed 2026-07-19** (fable subagent, gates re-verified): new `useCanvasDraft` (436L) replaces `useAIStreamingCanvasPreview` + TTADialog's tag trio — one record `{generationId, elementIds, targetCenter, queuedReplacementTags}`, API `applyChunk/applyFinal/commitDraft/queueReplacement/clearDraft/reset` (+`cancelPendingRenders` for cancel-and-replace teardown); canvas keyed by the **local** `message.id` everywhere (server `messageId` metadata-only, `ai-delete-` fallback gone, lifecycle renders chunks without waiting for `started`); new shared `throttle.ts` with trailing flush (**M1 fixed**, pinned by test); **N3 fixed** (`isIntermediatePreviewElement` exported, excalidraw-app LocalData saves filtered via `getPersistableElements`, dup key constant deleted, mount+open orphan sweep). Sound deviations: queue self-filter removed (error-retry reuses its id — filter would strand the failed partial; pinned by test), capture-rule unified (IMMEDIATELY→`syncActionResult`, NEVER→`updateScene`, rule comment at both sites), sweep runs on panel open too (scene loads async). All §3 pinned invariants re-asserted in the rewritten tests. 54 passing + the 2 known, typecheck clean. Live-QA follow-ups: error-retry replacement timing, undo grouping after the capture flip, reload-mid-stream orphan check.

- ✅ **Steps 3+4 landed 2026-07-18** (implemented by a fable subagent against §2.1/§2.5; independently verified): the §2.1 union is in — `ChatMessage = UserMessage | AssistantMessage | SystemWarningMessage`, `AssistantStatus` discriminated union, `server?: {turnId, messageId}` metadata, all 17 patch sites now single `status =` assignments, rate-limit bubbles are real system messages (`TTADialogRenderWarning` signature updated; assistant 429 errors synthesize one so the host upsell banner keeps working), truncation renders from `status.warning`. Flat persistence: `ChatConversation.messages`, round-trip trio + `ChatTurn`/`AssistantChatTurnMessage` deleted (−157 LOC in chatHelpers), per-chat IndexedDB keys with the new `{loadChats, saveChat, deleteChat}` adapter, buffer-new-chat-until-`started` (the `applyServerChatId` id-swap branch + `chatIdRef` are gone), terminal-only assistant persistence with an identity-stable memo so streaming never re-serializes, hydrate merges by id+`updatedAt` (M3), terminal error bubbles persist, warnings don't. New `useTTAChatHistory.test.tsx` (hydrate-merge, buffer-until-chatId + terminal-only policy, delete→adapter); lifecycle tests ported. Notable deviations (all sound): legacy `?? messageId` retry fallback dropped (data wiped; and regenerate now seeds `lastCompletedMessageId` from the source's — fixes seeding a never-completed attempt id), thumbnail renderKey simplified to id+theme+streaming (step-6 territory but old key read deleted fields), rename is the one non-active-chat write (parity). **Wipe dev IndexedDB once** — the legacy `"all"` blob is ignored, not migrated. 43 tests passing, typecheck clean.
...
7. **TODO — re-run should cancel the orphaned in-flight generation server-side** (companion to the §5.8 client re-run affordance, 2026-07-19): a reload mid-generation abandons the SSE response but the server generation keeps running and saves its result to the chat unseen; when the user re-runs the prompt-only turn, the chat can end up with both the orphan's result and the re-run's. Once the server is editable: let the re-run request (or a dedicated cancel endpoint) abort the in-flight generation for that chat/turn before starting the fresh turn. Related: server-side chat GET (loading a finished chat's turns from the server rather than local persistence) remains out of client scope entirely.
8. **TODO — stop streaming empty `partial` keep-alive frames** (S3 seen from the client side): the server emits continuous empty `partial` frames (~8/s, observed 20–31 before the first content chunk on dev) — pure wire waste, and the cadence is what exposed the client's status-label flashing (fixed client-side 2026-07-19, `0090d7e23`, via guarded no-op status patches). The S3 throttle fix should suppress empty/non-additive partials entirely; if a liveness signal is still wanted, use SSE comment-line heartbeats (item 3) which the client parser already tolerates and which never reach the app layer.
